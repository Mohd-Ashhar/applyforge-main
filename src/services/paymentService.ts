...
export const createRazorpayOrder = async (
  ...
  const response = await fetch('https://primary-production-800d.up.railway.app/webhook-test/create-razorpay-order', {
    method: 'POST',
    headers: {
...
export const updateUserSubscription = async (
  ...
  if (error) {
    ...
    throw new Error("Payment successful but subscription update failed. Please contact support.");
  }
...
export const upgradeToPlan = async (plan: SubscriptionPlan, userEmail: string) => {
  if (plan === 'Free') {
