// supabase/functions/auto-apply-proxy/index.ts
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (authError || !user) {
      throw new Error('Authentication required to use the auto-apply agent.')
    }

    // 2. Get the n8n webhook URL from secure environment variables
    const n8nWebhookUrl = Deno.env.get('N8N_AUTO_APPLY_WEBHOOK_URL')
    if (!n8nWebhookUrl) {
